...
// Mesma interface usada em ObraPlusPage
...
export default function LeadsTab() {
  ...
  React.useEffect(() => {
    loadUserState().then((userData) => {
      ...
      if (cities.length) {
        ...
        // Seleciona cidade do localStorage ou primeira da lista
  ...
  // Busca bairros quando cidade muda
  React.useEffect(() => {
  ...
  // Busca leads (obras favoritadas) quando filtros mudam
  React.useEffect(() => {
    if (!selectedCity) return;
    ...
    const fetchLeads = async () => {
      ...
      try {
        ...
        // Use the new endpoint for all possible leads
    ...
    fetchLeads();
  }, [selectedCity, selectedNeighborhood, selectedOrder, offset, rowsPerPage, refresh, filterValue]);

  const doRefresh = () => setRefresh((r) => r + 1);

  // Toggle favorito (remove lead)
  const toggleFavorite = async (obra_id: string) => {
    try {
      ...
      // Remove da lista local (já que deixou de ser favorito)
  ...
  const leadTemplate = (row: ResultRecord) => (
    <div>
      <div className="flex align-items-center gap-2">
        <strong style={{ fontSize: 14 }}>{row.owner || row.professional || "Sem nome"}</strong>
        {row.owner_contacted_at || row.professional_contacted_at ? (
          <i className="pi pi-check-circle" style={{ color: "#22C55E", fontSize: 14 }} title="Já contactado" />
        ) : null}
        <i className="pi pi-star-fill" style={{ color: "#F59E0B", fontSize: 14 }} title="Lead" />
      </div>
      {row.professional && row.owner && (
        <div className="text-secondary text-sm">{row.professional}</div>
      )}
    </div>
  );

  const contatoTemplate = (row: ResultRecord) => (
    <div style={{ lineHeight: 1.7 }}>
      {(row.has_owner_phone || row.has_professional_phone) && (
        <div className="flex align-items-center gap-2 text-secondary">
          <i className="pi pi-phone" />
          <span>{row.has_owner_phone ? "Proprietário" : ""}{row.has_owner_phone && row.has_professional_phone ? " / " : ""}{row.has_professional_phone ? "Profissional" : ""}</span>
        </div>
      )}
      {(row.has_owner_email || row.has_professional_email) && (
        <div className="flex align-items-center gap-2 text-secondary">
          <i className="pi pi-envelope" />
          <span>{row.has_owner_email ? "Proprietário" : ""}{row.has_owner_email && row.has_professional_email ? " / " : ""}{row.has_professional_email ? "Profissional" : ""}</span>
        </div>
      )}
      {!row.has_owner_phone && !row.has_professional_phone && !row.has_owner_email && !row.has_professional_email && (
        <span className="text-secondary">Sem contato</span>
      )}
    </div>
  );

  const localTemplate = (row: ResultRecord) => (
    <div className="flex align-items-center gap-2 text-secondary">
  ...
  const obraTemplate = (row: ResultRecord) => (
    <div style={{ lineHeight: 1.7 }}>
      <div className="flex align-items-center gap-2 text-secondary">
        <i className="pi pi-home" />
        <span>{row.type?.split(" - ")[1] || row.type || "N/A"}</span>
      </div>
      <div className="text-secondary">{row.size?.toFixed(0) || 0}m²</div>
    </div>
  );

  const statusTemplate = (row: ResultRecord) => {
    const contacted = row.owner_contacted_at || row.professional_contacted_at;
    const pending = row.owner_contact_pending_at || row.professional_contact_pending_at;
    
    if (contacted) {
      return <Tag value="Contactado" severity="success" className="border-round-xl" />;
    }
    if (pending) {
      return <Tag value="Pendente" severity="warning" className="border-round-xl" />;
    }
    return <Tag value="Novo" severity="info" className="border-round-xl" />;
  };

  const acoesTemplate = (row: ResultRecord) => (
    ...
    <div className="w-full">
      ...
      <div className=" border-round-3xl p-2 surface-border">
        {loading ? (
          ...
          <div className="text-center py-5 text-secondary">Nenhum lead encontrado. Não há obras disponíveis para contato nesta região.</div>
        ) : (
          <div className="grid">
            {records.map((row) => (
              <div key={row.obra_id} className="col-12 md:col-6 lg:col-4 xl:col-3 p-2">
                <div className="border-round-xl border-1 surface-border p-3 h-full flex flex-column justify-content-between">
                  <div className="flex align-items-center gap-2 mb-2">
                    <strong style={{ fontSize: 16 }}>{row.owner || row.professional || "Sem nome"}</strong>
                    {row.owner_contacted_at || row.professional_contacted_at ? (
                      <i className="pi pi-check-circle" style={{ color: "#22C55E", fontSize: 16 }} title="Já contactado" />
                    ) : null}
                    <i className="pi pi-star-fill" style={{ color: "#F59E0B", fontSize: 16 }} title="Lead" />
                  </div>
                  <div className="mb-2 text-secondary">
                    <i className="pi pi-map-marker" /> {row.city}, {row.bairro}
                  </div>
                  <div className="mb-2 text-secondary">
                    <i className="pi pi-home" /> {row.type?.split(" - ")[1] || row.type || "N/A"} | {row.size?.toFixed(0) || 0}m²
                  </div>
                  <div className="mb-2">
                    {(row.has_owner_phone || row.has_professional_phone) && (
                      <span className="flex align-items-center gap-2 text-secondary">
                        <i className="pi pi-phone" />
                        <span>{row.has_owner_phone ? "Proprietário" : ""}{row.has_owner_phone && row.has_professional_phone ? " / " : ""}{row.has_professional_phone ? "Profissional" : ""}</span>
                      </span>
                    )}
                  ...
                  <div className="mb-2">
                    ...
                    ) : row.owner_contact_pending_at || row.professional_contact_pending_at ? (
